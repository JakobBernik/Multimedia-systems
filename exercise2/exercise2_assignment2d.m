...
[h,w,c,n]=size(video);
v=VideoWriter('SmoothBuck.avi','Uncompressed AVI');
open(v);
Hues=zeros(h, w, 1, n);
trackX=zeros(n-1);
trackY=zeros(n-1);
...
for i=1:n-1
    ...
    [smoothX,smoothY]=smooth_trajectory(trackX,trackX);
    for i=1:n
     xmin=min(max(smoothX(i)-50, 1), h-100); % poskrbi da je dobljeni kvadrat vedno 100*100
     ymin=min(max(smoothY(i)-50, 1), w-100);
